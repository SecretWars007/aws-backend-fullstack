  ...
  endpoints.forEach((url) => {
    describe(`POST ${url}`, () => {
      it('returns 200 on valid user data', async () => {
        const res = await request(app).post(url).send(withAuth(validPayload));
        expect(res.status).toBe(200);
        expect(res.body.state).toBe(0);
      ...
      it('returns 400 when cellphone is missing', async () => {
        ...
        const res = await request(app).post(url).send(withAuth(noPhone));
        expect(res.status).toBe(400);
      });

      it('returns 400 when email is missing', async () => {
        const { email: _, ...noEmail } = validPayload;
        const res = await request(app).post(url).send(withAuth(noEmail));
        expect(res.status).toBe(400);
      });
...
describe('Customer Service - Register Reference Code', () => {
  ...
  endpoints.forEach((url) => {
    ...
    it(`POST ${url} → 400 when id is missing`, async () => {
      const res = await request(app).post(url).send(withAuth({ code: 'REF-2024' }));
      expect(res.status).toBe(400);
    });
  ...
  endpoints.forEach((url) => {
    describe(`POST ${url}`, () => {
      ...
      it('returns 400 when mobile_number is missing', async () => {
        ...
        const res = await request(app).post(url).send(withAuth(noMobile));
        expect(res.status).toBe(400);
      });

      it('returns 400 when pin is missing', async () => {
        const { pin: _, ...noPin } = validPayload;
        const res = await request(app).post(url).send(withAuth(noPin));
        expect(res.status).toBe(400);
      });
...
describe('Customer Service - Internal Routes', () => {
  it(`GET /internal/customer/phone/${SEED_CELLPHONE} → 200 with seeded data`, async () => {
    const res = await request(app).get(`/internal/customer/phone/${SEED_CELLPHONE}`);
    expect(res.status).toBe(200);
  ...
  it('GET /internal/customer/phone/00000001 → 404 (unknown number)', async () => {
    const res = await request(app).get('/internal/customer/phone/00000001');
    expect(res.status).toBe(404);
  });
});
...
describe('Customer Service - 404 Not Found', () => {
  it('returns 404 for unknown endpoints', async () => {
    const res = await request(app).post('/V1/completely-unknown');
    expect(res.status).toBe(404);
  });
});
