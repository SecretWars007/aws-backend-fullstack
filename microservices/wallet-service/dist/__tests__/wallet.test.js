...
function makeAuthToken(certifiedId = 3) {
    return jsonwebtoken_1.default.sign({ deviceId: 'test-device-001', certifiedId }, 'zappi-device-secret-CHANGE-IN-PROD', { expiresIn: '1h' });
}
    ...
    endpoints.forEach((url) => {
        describe(`POST ${url}`, () => {
            it('returns 200 on valid Entel recharge', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth(validPayload, 3));
                expect(res.status).toBe(200);
                expect(res.body.state).toBe(0);
            ...
            it('returns 400 on missing cellphone', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth({ amount: 20 }));
                expect(res.status).toBe(400);
            });
            it('returns 400 on missing amount', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth({ cellphone: '71234567' }));
                expect(res.status).toBe(400);
            });
            it('returns 401 without auth_token', async () => {
...
describe('Wallet Service - Recharge Tigo', () => {
    ...
    endpoints.forEach((url) => {
        describe(`POST ${url}`, () => {
            ...
            it('returns 400 on missing cellphone', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth({ amount: 15 }));
                expect(res.status).toBe(400);
            });
            it('returns 401 without auth_token', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(validPayload);
...
describe('Wallet Service - Recharge Viva', () => {
    ...
    endpoints.forEach((url) => {
        describe(`POST ${url}`, () => {
            ...
            it('returns 400 on missing amount', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth({ cellphone: '73456789' }));
                expect(res.status).toBe(400);
            });
            it('returns 401 without auth_token', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(validPayload);
...
describe('Wallet Service - Transfer Validate', () => {
    ...
    endpoints.forEach((url) => {
        describe(`POST ${url}`, () => {
            ...
            it('returns 401 without auth_token', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send({ cellphone: '70000099' });
                expect(res.status).toBe(401);
            });
        });
    });
...
describe('Wallet Service - Transfer Token Generate', () => {
    ...
    endpoints.forEach((url) => {
        describe(`POST ${url}`, () => {
            ...
            it('returns 400 when cellphone is missing', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth({ amount: 50 }));
                expect(res.status).toBe(400);
            });
            it('returns 400 when amount is missing', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(withAuth({ cellphone: '70000099' }));
                expect(res.status).toBe(400);
            });
            it('returns 401 without auth_token', async () => {
                const res = await (0, supertest_1.default)(app).post(url).send(validPayload);
...
describe('Wallet Service - Transfer Execute', () => {
    ...
    it('Full transfer flow: token generate → execute → 200', async () => {
        ...
        const tokenRes = await (0, supertest_1.default)(app)
...
describe('Wallet Service - 404 Not Found', () => {
    it('returns 404 for unknown endpoints', async () => {
        const res = await (0, supertest_1.default)(app).post('/V1/completely-unknown-endpoint');
